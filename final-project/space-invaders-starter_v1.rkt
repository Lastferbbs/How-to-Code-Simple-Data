...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname space-invaders-starter_v1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (advance-game game)
  (make-game empty empty (move-tank (game-tank game))))
...
(check-expect (control-tank (make-game empty empty (make-tank 50 -1)) "left") (make-game empty empty (make-tank 50 -1)))
(check-expect (control-tank (make-game empty empty (make-tank 50 -1)) "right") (make-game empty empty (make-tank 50 1)))
(check-expect (control-tank (make-game empty empty (make-tank 50 1)) " ") (make-game empty empty (make-tank 50 1)))


; (define (control-tank tank key) empty-image) ;stub

(define (control-tank game key)
  (cond [(key=? key "right")(make-game empty empty (make-tank (tank-x (game-tank game)) 1))]
        [(key=? key "left")(make-game empty empty (make-tank (tank-x (game-tank game)) -1))]
        [else (make-game empty empty (game-tank game))]))

;; Game -> Game
;; Add new missile to listof Missile in game
(check-expect (launch-missile (make-game empty empty (make-tank 100 1))) (make-game empty (list (make-missile 10 10)) (make-tank 100 1)))

; (define (launch-missile game) (make-game empty empty empty))

(define (launch-missile game)
  (list-append 
...
;; Tank -> Image
;; render tank
(check-expect (render-tank (make-tank 100 1)) (place-image TANK 100 (- HEIGHT TANK-HEIGHT/2) BACKGROUND))
